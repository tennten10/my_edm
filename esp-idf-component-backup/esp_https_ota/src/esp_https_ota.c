...
//if (!is_server_verification_enabled(ota_config)) {
    ...
    const int alloc_size = (ota_config->http_config->buffer_size > DEFAULT_OTA_BUF_SIZE ?  ota_config->http_config->buffer_size : DEFAULT_OTA_BUF_SIZE); //MAX(ota_config->http_config->buffer_size, DEFAULT_OTA_BUF_SIZE);
